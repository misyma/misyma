...
import { type FilterOpts } from '../types/filter';
...
type DynamicFilterContextType = {
  ...
  updateFilterValue: (id: PropertyKey, value: string | boolean | number | Date | undefined) => void;
...
export const DynamicFilterProvider: FC<DynamicFilterProviderProps> = ({
  ...
  const updateFilterValue = (key: PropertyKey, value: string | boolean | number | Date | undefined) => {
    if (value === undefined) {
      setFilterValues((prev) => {
        delete prev[key as string];
...
export const useDynamicFilterContext = () => {
  ...
  if (context === undefined) {
    throw new Error('useDynamicFilterContext must be used within a DynamicFilterProvider');
  }
