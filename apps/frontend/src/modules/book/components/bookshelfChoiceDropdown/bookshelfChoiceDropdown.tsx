...
export const BookshelfChoiceDropdown: FC<Props> = ({
  bookId,
  currentBookshelfId,
}) => {
  ...
  const [previousBookshelfId, setPreviousBookshelfId] = useState<string | null>(
    null
  );

  const booksBookshelf = useMemo(() => {
    return bookshelfData?.data.find(
      (bookshelf) => data?.bookshelfId === bookshelf.id
    );
  }, [bookshelfData, data]);
  ...
  useEffect(() => {
    setCurrentBookshelf(
      bookshelfData?.data.find(
        (bookshelf) => bookshelf.id === currentBookshelfId
      )?.name ?? ''
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentBookshelfId]);
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent>
          <Command shouldFilter={false}>
            <CommandInput placeholder="" onValueChange={setSearchedName} />
            <CommandList>
              {
                <>
                  ...
                  {isLoading && (
                    <CommandLoading>Wyszukuję półki</CommandLoading>
                  )}
                  <CommandGroup>
          ...
          onMutated={async () => {
            ...
            await Promise.all([
              queryClient.invalidateQueries({
                predicate: ({ queryKey }) =>
                  queryKey[0] ===
                    BorrowingApiQueryKeys.findBookBorrowingsQuery &&
                  queryKey[1] === bookId,
              }),
              queryClient.invalidateQueries({
                queryKey: [
