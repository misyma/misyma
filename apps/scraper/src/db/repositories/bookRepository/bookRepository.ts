...
export class BookRepository {
  ...
  public async createBook(payload: CreateBookPayload): Promise<void> {
    ...
    try {
      await this.databaseClient.transaction(async (transaction) => {
        await transaction<Book>(bookTable).insert({
          id,
          title,
          isbn,
          publisher,
  ...
  public async updateBook(payload: UpdateBookPayload): Promise<void> {
    const { id, title, isbn, publisher, releaseYear, language, translator, format, pages, imageUrl } = payload;

    let updatePayload: Partial<Book> = {};
    ...
    try {
      await this.databaseClient<Book>(bookTable).update(updatePayload, '*').where({ id });
    } catch (error) {
      throw new RepositoryError({
        entity: 'Book',
        operation: 'update',
  ...
  public async findBook(payload: FindBookPayload): Promise<Book | null> {
    ...
    try {
      ...
      const book = await this.databaseClient<Book>(bookTable).select('*').where(whereClause).first();
